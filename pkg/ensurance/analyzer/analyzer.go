...
func (s *AnormalyAnalyzer) getThrottlePods(enableSchedule bool, ac ecache.ActionContext,
	...
	for _, pod := range allPods {
		if ac.Triggered {
			throttlePods = append(throttlePods, executor.PodBasicInfoConstruct(pod, stateMap, action, executor.Throttle))
		}
		if enableSchedule && ac.Restored {
			throttleUpPods = append(throttleUpPods, executor.PodBasicInfoConstruct(pod, stateMap, action, executor.Throttle))
		}
	}
...
func (s *AnormalyAnalyzer) getEvictPods(triggered bool, action *ensuranceapi.AvoidanceAction, stateMap map[string][]common.TimeSeries) []executor.PodContext {
	...
	if triggered {
		...
		for _, pod := range allPods {
			evictPods = append(evictPods, executor.PodBasicInfoConstruct(pod, stateMap, action, executor.Evict))
