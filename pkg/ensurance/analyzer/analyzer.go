...
func (s *AnormalyAnalyzer) getThrottlePods(enableSchedule bool, ac ecache.ActionContext,
	...
	for _, pod := range allPods {
		if ac.Triggered {
			throttlePods = append(throttlePods, podinfo.PodBasicInfoConstruct(pod, stateMap, action, podinfo.ThrottleDown))

		}
		if enableSchedule && ac.Restored {
			throttleUpPods = append(throttleUpPods, podinfo.PodBasicInfoConstruct(pod, stateMap, action, podinfo.ThrottleUp))
		}
	}
...
func (s *AnormalyAnalyzer) getEvictPods(triggered bool, action *ensuranceapi.AvoidanceAction, stateMap map[string][]common.TimeSeries) []podinfo.PodContext {
	...
	if triggered {
		...
		for _, pod := range allPods {
			evictPods = append(evictPods, podinfo.PodBasicInfoConstruct(pod, stateMap, action, podinfo.Evict))
		}
	}
	return evictPods
}
